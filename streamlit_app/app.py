...
# Page Config
# -------------------------------
st.set_page_config(page_title="🎧 ListenBrainz Assignment Dashboard", layout="wide")
...
st.markdown(
    "This dashboard answers the challenge using the **reporting views** built by Airflow over DuckDB."
)
...
with tab1:
    ...
    if df.empty:
        ...
        user_col = pick_col(
            df, preferred=["user_id", "user_name", "user"], contains_any=["user"]
        )
        listens_col = pick_col(
            df,
            preferred=["number_of_listens", "listens", "count"],
            contains_any=["listen", "count"],
        ...
        if user_col and listens_col:
            ...
            st.error(
                f"Couldn't find appropriate user/listen columns. user_col={user_col}, listens_col={listens_col}"
            )
    ...
    if df.empty:
        ...
        if user_col:
            sel_user = st.selectbox(
                "Filter by user",
                ["All"] + sorted(df[user_col].astype(str).unique().tolist()),
            )
            if sel_user != "All":
...
with tab4:
    ...
    if df.empty:
        ...
        date_col = pick_col(
            df, preferred=["date", "listened_date", "dt"], contains_any=["date", "day"]
        )
        if user_col and listens_col and date_col:
            df_sorted = df.sort_values([user_col, listens_col], ascending=[True, False])
            pick = st.selectbox(
                "Select user", sorted(df[user_col].astype(str).unique().tolist())
            )
            ...
            st.error(
                f"Couldn't find user/listens/date columns. user_col={user_col}, listens_col={listens_col}, date_col={date_col}"
            )
...
with tab5:
    ...
    if df.empty:
        ...
        show_cols_hint(df)
        date_col = pick_col(
            df, preferred=["date", "dt", "day"], contains_any=["date", "day"]
        )
        active_col = pick_col(
            df,
            preferred=["number_active_users", "active_users"],
            contains_any=["active", "users"],
            require_numeric=True,
        )
        pct_col = pick_col(
            df,
            preferred=["percentage_active_users", "pct_active"],
            contains_any=["percent", "pct"],
            require_numeric=True,
        )
        if date_col and active_col:
            ...
            st.error(
                f"Couldn't find date/active columns. date_col={date_col}, active_col={active_col}, pct_col={pct_col}"
            )
